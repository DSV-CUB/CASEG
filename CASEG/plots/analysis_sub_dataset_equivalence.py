...
for i in range(len(models)):
    ...
    for j in range(len(data_generators)):
        ...
        predictions_model.append(copy.copy(prediction_value))
        expectations_model.append(copy.copy(expectation_value))
...
for i in range(len(models_names)):
    information = {"models_name": sub_dirs,
                   "predictions_values": predictions[i],
                   "expectations_values": expectations[i],
                   "equivalence": data_reference}

    t1_equivalence.plot(information, axes[i,:].flatten(), bonferoni=False, nobias=nobias)
